...
def date_converter(map, string):
    first_backslash = string.find("/")
    second_backslash = string.find("/", first_backslash+1)
    month = string[:first_backslash]
    day = string[first_backslash+1:second_backslash]
    year = string[second_backslash+1:]
    
    if map == english:
        return day + " " + map[int(month)] + " " + year
    return day + " " + map[int(month)] + " " + year
